- view: client_last_system_program_enrollment
  derived_table:
    sql: |
      select cp.id, cp.ref_client, cp.ref_program from client_programs cp INNER JOIN 
      (select  ref_client, max(start_date) as last_start from client_programs where deleted is null group by 1) latest_cp
